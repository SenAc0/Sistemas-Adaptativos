...
vector<int> misp_heuristic_randomized(const vector<vector<int>>& adjList, float UMBRAL_IN, int k) {
    ...
    while (true) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            for (int j = 0; j < k; ++j) {
                if (d < k_degs[j]) {
                    // desplazar hacia abajo para insertar aquí
                    for (int m = k - 1; m > j; --m) {
                        k_degs[m] = k_degs[m - 1];
...
void RunTxtFile(string txt) {
    ...
    for (int i = 0; i < file_paths.size(); i++) {
        ...
        result[i % 30] = misp_heuristic_randomized(adjList,0.7,4).size();
        auto end = high_resolution_clock::now();

        bool validate_console;
        validate_console = validate_independent_set(adjList, misp_heuristic_randomized(adjList,0.7,4));
...
int main(int argc, char* argv[]) {
    if (argc < 3) {
        cerr << "Uso: " << argv[0] << " -i <instancia-problema> [-u umbral] [-k k]" << endl;
        return 1;
    }
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (opt == "-u" && i + 1 < argc) {
            umbral = stof(argv[++i]);
        } else if (opt == "-k" && i + 1 < argc) {
            k = stoi(argv[++i]);
        }
    }

    if (instance_path.empty()) {
        cerr << "Error: no se especificó instancia con -i" << endl;
        return 1;
    }

    RunInstanceProb(instance_path, umbral, k);
    return 0;
}
